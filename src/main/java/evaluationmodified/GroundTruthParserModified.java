...
public class GroundTruthParserModified {

	public static GroundTruthFileModified parse(String fileName){
		try{
			...
			final GroundTruthFileModified groundTruthFile = new GroundTruthFileModified();
			...
			if(contentNode.hasChildNodes()){
				for(int i=0;i<contentNode.getChildNodes().getLength();i++){
					...
					if (n.getNodeType() == Node.ELEMENT_NODE) {
						...
						groundTruthFile.addData(tagValue,category, entityValue);
					}
				}
			}
	...
	public static GroundTruthFileModified parseText(String text) {
		try{
			DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
			DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
