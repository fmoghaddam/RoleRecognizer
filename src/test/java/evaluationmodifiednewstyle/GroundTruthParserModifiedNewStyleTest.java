...
public class GroundTruthParserModifiedNewStyleTest {
	...
	public void testGetTokenNumber(){
		...
		Position result = GroundTruthParserModifiedNewStyle.getTokenNumberStanfordTokenizer(test, new Position(10,23));
		assertEquals(4,result.getStartIndex());
		assertEquals(6,result.getEndIndex());
		
		result = GroundTruthParserModifiedNewStyle.getTokenNumberStanfordTokenizer(test, new Position(0,1));
		assertEquals(1,result.getStartIndex());
		assertEquals(1,result.getEndIndex());
		
		result = GroundTruthParserModifiedNewStyle.getTokenNumberStanfordTokenizer(test, new Position(10,17));
		assertEquals(4,result.getStartIndex());
		...
		result = GroundTruthParserModifiedNewStyle.getTokenNumberStanfordTokenizer(test2, new Position(169,189));
		assertEquals(31,result.getStartIndex());
		assertEquals(33,result.getEndIndex());
		
		result = GroundTruthParserModifiedNewStyle.getTokenNumberStanfordTokenizer(test2, new Position(685,701));
		assertEquals(130,result.getStartIndex());
		assertEquals(132,result.getEndIndex());
		
		final String test3 = "WANT to peek at a family Web site crammed with murder, treachery, blood-lust, centuries-long feuds, torture, insanity and whole dungeonfuls of assorted other skeletons? The <MISC> monarchy is just the ticket (www.royal.gov.uk).  Not the current entries, of course. Since the royal family entered cyberspace in 1997 to help satisfy the public's insatiable curiosity, its official Web realm has compiled a ream's worth of stars and garters, palaces and Plantagenets. But there is more on the site about the two-penny <MISC> plum stamp of 1910 in the royal philatelic collection than about <PERSON> <PERSON>. (Only one canceled copy of each is known to exist.)  The pages devoted to <PERSON>, Princess of <LOCATION>, are suitably prominent, reverential and respectful. The pages detailing a typical day in the life of the queen depict a very wealthy, very earnest, very busy woman. But the mountains of minutiae seem to highlight the obvious omissions, and to make them more poignant. Duty. Dedication. Few tears. Little joy.  Royal Insight, the site's online magazine, highlights important engagements of the month. It will not be mistaken for a tabloid. (May's feature: visiting the <LOCATION> flower show.) There is enough data about castles, remodeling, acreage, equerries, swan upping, royal mews and queen's beasts to put even the most ardent monarchist to sleep.  The casual viewer who scans the site quickly for the trivia will unearth some gems:  Prince <PERSON> began his education at Mrs. <PERSON>'s <ORGANIZATION> <ORGANIZATION> in September 1985, at age 3.  The last Welsh Prince of <LOCATION> was <PERSON> ap Gruffyd (1246-82).  <PERSON>'s oldest child, <PERSON>, the Princess Royal, was the mother of <PERSON> <PERSON> <PERSON>.  The royal train has two locomotives. They are named Prince <PERSON> and Prince <PERSON>.  The royal stamp collection (beloved especially by <PERSON> <PERSON>) includes what is believed to be the personal stamp album of the murdered <PERSON> <PERSON>, heir to the <MISC> throne. The brightly illustrated child's album was stolen by a <MISC> soldier and sold to an employee of a <MISC> company in the <LOCATION> <LOCATION>.  The Britannia was the 83rd royal yacht.  <PERSON> I attended the premiere of ''A Midsummer Night's Dream.''  <PERSON> <PERSON>'s interest in botany earned him the nickname <PERSON> <PERSON>. (No mention of the nicknames he was given on this side of the pond.)  The site also has some unintentionally funny writing. ''<PERSON> <PERSON> is widely remembered for two things: losing the <MISC> colonies and going mad. This is far from the whole truth.''  The best part of the royal site is the deep past, starting with the <MISC> kings of <ORGANIZATION>. There are longer profiles of notable monarchs like <PERSON> the Great, <PERSON> the Conqueror, <PERSON> VIII (who wrote a best seller defending the <ORGANIZATION> <ORGANIZATION> <ORGANIZATION>, before all that marital unpleasantness changed things) and <PERSON> I (he stammered). As a capsule <MISC> history, the site isn't bad, as long as you don't look for too much information on how the common people lived.  But for incomparable mayhem, nothing beats the history of the <MISC> crown. Here is a sample from 962 to 1005: <PERSON> was killed by <PERSON> who was killed by <PERSON>, king of <LOCATION>, whose daughter he had seized. He was succeeded by <PERSON> <PERSON>, <PERSON>'s brother, who killed <PERSON>'s brother and was killed by <PERSON>'s son, <PERSON>. <PERSON>'s reign was short; he was killed, probably by <PERSON> <PERSON>I. <PERSON> was killed and succeeded by <PERSON> <PERSON>.  The glory of <LOCATION> isn't its pomp, but its language and its ideals. The speeches of some of its monarchs ring out from the site.  <PERSON> I to her army on the eve of the <MISC> <MISC>: ''I know I have the body of a weak, feeble woman; but I have the heart and stomach of a king -- and of a king of <LOCATION>, too.''  <PERSON> I proclaims an attitude: '' Kings are justly called gods, for that they exercise a manner of resemblance of divine power on earth.''  A member of <ORGANIZATION> at the time of the Glorious Revolution of 1688, on the importance of making <PERSON> and <PERSON> and future monarchs financially dependent on the legislative body: ''When princes have not needed money, they have not needed us.''";
		result = GroundTruthParserModifiedNewStyle.getTokenNumberStanfordTokenizer(test3, new Position(702,712));
		assertEquals(132,result.getStartIndex());
		assertEquals(132,result.getEndIndex());
	}
}
